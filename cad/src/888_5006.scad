...
module 888_5006_attachment_points() {
    // tower arms /////////////////////////////////////////////////////////
    translate([0, base_width/2-ALU_profile_width, 35])
    ...
    // tower hinges
    translate([ALU_profile_width*.75, -base_width/2+ALU_profile_width+ALU_profile_holder_wall_thickness*2, -ALU_profile_width-ALU_profile_holder_wall_thickness])
    rotate([0, -90 ,0])
    888_5005();
    ...
    // tower hinges axles
    translate([0, -base_width/2-ALU_profile_holder_wall_thickness*2, (608_bearing_outer_diameter+10)/2])
    rotate([90, 0, 180])
    cylinder(d=8, h=50);
...
module 888_5006(side=1) {
    difference() {
        hull() {
            // hinge cylinder
            translate([0, (-base_width/2+ALU_profile_width/2+ALU_profile_holder_wall_thickness*2)*side+ALU_profile_width/2+ALU_profile_holder_wall_thickness*4 ,(608_bearing_outer_diameter+10)/2])
            rotate([90, 0, 0])
            cylinder(d=608_bearing_outer_diameter+10, h=ALU_profile_width+ALU_profile_holder_wall_thickness*8, $fn=100);
            
            
            translate([-ALU_profile_width/2-ALU_profile_holder_wall_thickness*2, (-base_width/2+ALU_profile_width)*side, 35])
        ...
        // profile arm cutter
        translate([-ALU_profile_width/2, (-base_width/2+ALU_profile_width)*side, 35])
        rotate([-tower_angle*side, 0 ,0])
        union() {
        ...
        // hinge inside cutter
        translate([-ALU_profile_width, (-base_width/2-M6_washer_thickness+ALU_profile_width/2+ALU_profile_holder_wall_thickness*1.5)*side-ALU_profile_width/2-ALU_profile_holder_wall_thickness*1.5 ,0])
        ...
        // hinge screw cutter
        translate([0, (-base_width/2-ALU_profile_holder_wall_thickness*2-.1)*side,(608_bearing_outer_diameter+10)/2])
